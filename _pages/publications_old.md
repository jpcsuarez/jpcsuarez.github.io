...
<div id="filters">
  <button type="button" onclick="filterSelection('all')">All</button>
  <button type="button" onclick="filterSelection('Transportation')">Transportation</button>
  <button type="button" onclick="filterSelection('SocialMedia')">Social Media &amp; Social Science</button>
</div>
...
<div id="filters-context">
  <button type="button" onclick="filterSelection('all')">All</button>
  <button type="button" onclick="filterSelection('Taiwan')">Taiwan</button>
  <button type="button" onclick="filterSelection('Qatar')">Qatar</button>
  <button type="button" onclick="filterSelection('Global')">Global</button>
  <button type="button" onclick="filterSelection('Philippines')">Philippines</button>
</div>
...
<div class="pub-item" data-topic="SocialMedia" data-context="Global">
  <p>Adrian Rauchfleisch, <strong>Joshua Philip Suarez</strong>, Nikka Marie Sales &amp; Andreas Jungherr (2025). Winning and losing with Artificial Intelligence: What public discourse about ChatGPT tells us about how societies make sense of technological change. <em>Telematics and Informatics</em><br>
...
function filterSelection(filter) {
  const items = document.getElementsByClassName("pub-item");

  for (let i = 0; i < items.length; i++) {
    const topic = items[i].getAttribute("data-topic");
    const context = items[i].getAttribute("data-context");

    if (filter === "all" || topic === filter || context === filter) {
